...
namespace recorder
{
inline std::shared_ptr<GenericSubscription> create_generic_subscription(
  ...
  const std::function<void(std::shared_ptr<rclcpp::SerializedMessage>, uint64_t timestamp)>& callback)
{
  ...
  try {
    subscription = std::make_shared<GenericSubscription>(
      topics_interface->get_node_base_interface(), *type_support,
